...
extern void mem_free(void* address, uint32_t size);
...
#define LOST_UNINITIALIZED_MEM  0xDEADBEEF
#define LOST_FREED_MEM  0xD00FC0DE
...
struct liballoc_major
{
	...
	uintptr_t pages;					//< The number of pages in the block.
	uintptr_t size;					//< The number of pages in the block.
	uintptr_t usage;					//< The number of bytes used in the block.
...
struct	liballoc_minor
{
	...
	uintptr_t size; 					//< The size of the memory allocated. Could be 1 byte or more.
	uintptr_t req_size;				//< The size of memory requested.
};
...
static mutex_t liballoc_mutex = 0;

static int __liballoc_lock(void)
{
    mutex_lock(&liballoc_mutex);
	return 0;
}

static int __liballoc_unlock(void)
{
...
static void*	liballoc_fill_dword(void* s, uint32_t c, size_t n);
#endif

static void* liballoc_alloc( int pages )
{
	void* block = mem_allocate(0x1000 * pages, 0);
    
    #ifndef CONFIG_RELEASE_VERSION
    // Neu angeforderten Speicher vollschreiben, damit nicht faelschlich davon
    // ausgegangen wird, dass immer alles mit 0 initialisiert ist.
    liballoc_fill_dword(block, LOST_UNINITIALIZED_MEM, (0x1000 / 4) * pages);
    #endif

    return block;
}

static void liballoc_free( void* ptr, int pages )
{
	mem_free(ptr, 0x1000 * pages);
}
...
static struct liballoc_major *allocate_new_page( uintptr_t size )
{
	uintptr_t st;
...
void *PREFIX(malloc)(size_t req_size)
{
	...
	while ( maj != NULL )
	{
		...
		if ( maj->first == NULL )
		{
			...
			FLUSH();
			#endif
			__liballoc_unlock();		// release the lock
			return p;
		}
				

		// CASE 3: Block in use and enough space at the start of the block.
		diff = (uintptr_t)(maj->first) - (uintptr_t)maj - sizeof(struct liballoc_major);
		...
		while ( min != NULL )
		{
				...
				if ( min->next == NULL )
				{
					...
					diff -= ((uintptr_t) min + sizeof( struct liballoc_minor ) + min->size); 
				...
				if ( min->next != NULL )
				{
					...
					diff =  (uintptr_t)(min->next) - (uintptr_t)min  - sizeof( struct liballoc_minor ) - min->size;
...
void PREFIX(free)(void *ptr)
{
    ...
    #ifndef CONFIG_RELEASE_VERSION
    // Freigegebenen Speicher vollschreiben, damit es garantiert auffaellt,
    // wenn er nach einem free weiterbenutzt wird
    liballoc_fill_dword(ptr, LOST_FREED_MEM, min->size / 4);
    #endif
	...
	else
	{
		if ( l_bestBet != NULL )
		{
			uintptr_t bestSize = l_bestBet->size  - l_bestBet->usage;
			uintptr_t majSize = maj->size - maj->usage;
