/*
 * This code is part of the PDCLib, licensed under CC0
 */
...
#define CHECK_EQUAL( a, b ) do { int x = a; int y = b; if ( x != y ) { printf( "* FAIL %s: Mismatch in " symbol2string( __LINE__ ) ": result is %d, expected %d.\n", test_name, x, y ); failed++; } } while ( 0 )
#define CHECK_FEQUAL( a, b, T, F ) do { T x = a; T y = b; if ( x != y ) { printf( "* FAIL %s: Mismatch in " symbol2string( __LINE__ ) ": result is " F ", expected " F ".\n", test_name, x, y ); failed++; } } while ( 0 )
#define CHECK_NONZERO( a ) do { int x = (a); if ( x == 0 ) { printf( "* FAIL %s: Mismatch in " symbol2string( __LINE__ ) ": result is %d, expected non-zero.\n", test_name, x ); failed++; } } while ( 0 )

static int failed = 0;

// literal matches, character matches, and basic integer matches
static void suite_one( void ) __attribute__((used));
// decimal integer matches
static void suite_two( void ) __attribute__((used));
// hexadecimal integer matches
static void suite_three( void ) __attribute__((used));
// octal integer matches
static void suite_four( void ) __attribute__((used));
// string matches
static void suite_five( void ) __attribute__((used));

int test_stdio(void)
{
    failed = 0;

    chdir("ata:/ata00|ext2:/");
...
// literal matches, character matches, and basic integer matches
static void suite_one(void)
{
    FILE * file;
    if ( ( file = fopen( "tmpfile", "wb+" ) ) == NULL )
    {
        puts( "* ERROR Failed to open tmpfile for writing." );
        return;
    }

    CHECK_EQUAL( fprintf( file, "1234567890" ), 10);
    CHECK_EQUAL( fprintf( file, "1%c3-5+7%c9%c", 0, 0, -1 ), 10);
    CHECK_EQUAL( fprintf( file, "2 4 6 8 0%c", 255 ), 10);
    CHECK_EQUAL( fprintf( file, "1 \011 5%%%%  0" ), 10);
    ...
    char buffer[ 2 ];
    memset( buffer, '\177', 2 );
    CHECK_EQUAL( fscanf( file, "%2c", buffer ), 0 );
...
static void suite_two(void)
{
    ...
    int n;
    CHECK_EQUAL( fscanf( file, "%lld%n", &i, &n ), 1 );
    CHECK_FEQUAL( i, -9223372036854775808llu, signed long long, "%lld" ); // should be literal -9223372036854775808ll but GCC balks.
    CHECK_EQUAL( i < 0ll, 1 );
    CHECK_EQUAL( n, 20 );
    ...
    int n;
    CHECK_EQUAL( fscanf( file, "%lli%n", &i, &n ), 1 );
    CHECK_FEQUAL( i, -9223372036854775808llu, signed long long, "%lli" ); // should be literal -9223372036854775808ll but GCC balks.
    CHECK_EQUAL( i < 0ll, 1 );
    CHECK_EQUAL( n, 20 );
    CHECK_EQUAL( ftell( file ), 98 );
    }
...
static void suite_three(void)
{
    ...
    unsigned char i = -1;
    int n = -1;
    CHECK_EQUAL( fscanf( file, "%hhx%n", &i, &n ), 0 );
    CHECK_EQUAL( i, (unsigned char) -1 );
    CHECK_EQUAL( n, -1 );
    ...
    unsigned short i = -1;
    int n = -1;
    CHECK_EQUAL( fscanf( file, "%hx%n", &i, &n ), 0 );
    CHECK_EQUAL( i, (unsigned short) -1 );
    CHECK_EQUAL( n, -1 );
    CHECK_EQUAL( ftell( file ), 145 );
    }
    {
    ...
    unsigned int i = -1;
    int n = -1;
    CHECK_EQUAL( fscanf( file, "%x%n", &i, &n ), 0 );
    CHECK_EQUAL( i, (unsigned int) -1 );
    CHECK_EQUAL( n, -1 );
    CHECK_EQUAL( ftell( file ), 145 );
    }
    {
    ...
    int n;
    CHECK_EQUAL( fscanf( file, "%x%n", &i, &n ), 1 );
    CHECK_FEQUAL( i, 4294967295u, unsigned int, "%d" );
    CHECK_EQUAL( n, 8 );
