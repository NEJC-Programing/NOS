...
/**
 * Test: extend_subdir_file
 * Autor: Kevin Wolf
 * Zweck: Datei in einem Unterverzeichnis vergroessern
 */
int run_test(struct fat_fs* fs)
{
    ...
    ret = fat_file_open(fs, "/dir/foo.txt", &file);
    if (ret < 0) {
        test_error("Konnte Datei /dir/foo.txt nicht oeffnen");
    }
    ...
    ret = fat_file_open(fs, "/dir/foo.txt", &file);
    if (ret < 0) {
        test_error("Konnte Datei /dir/foo.txt nicht neu oeffnen");
    }
